...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`relative flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-
    [#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl 
    transition-all duration-500 max-md:absolute left-0 z-10 ${!isMenuOpen && 
    'max-md:translate-x-full'}`}>
      {/* Logo */}
      <img src={theme === 'dark' ? assets.logo_full : assets.logo_full_dark} alt=""
      className='w-full max-w-81' />

      {/* New Chat Button */}
      <button className='flex justify-center items-center w-full py-2 mt-10
      text-white bg-gradient-to-r from-[#4f1a87] to-[#6793df] text-sm rounded-md
      cursor-pointer'>
      ...
      hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
            ...
            <p className='text-xs text-gray-400'>Purchase credits to use SEVA</p>
        </div>
      </div>


      {/* Dark Mode Toggle */}
      <div onClick={()=> {navigate('/community')}} className='flex items-center justify-between gap-2
      ...
      p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
      ...
      cursor-pointer md:hidden not-dark:invert z-20' alt="close" />
